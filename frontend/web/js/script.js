...
if (is_video == true){
  ...
  this.root.querySelector('#story_media').src = apiEndpoint + "media/" + story.media + ".mp4"
  this.root.querySelector('#story_media').poster = apiEndpoint + "media/" + story.media + ".jpg"
...
this.root.querySelector('#story_avatar').style.backgroundImage = `url(${apiEndpoint}profile/${story.user}.jpg)`
this.root.querySelector('#story_avatar_big').src = apiEndpoint + "profile/" + story.user + ".jpg"
  ...
  const loadStories = stories.map((story) => {
    ...
    if (story.is_video == true){
      response["media"] = new Promise((resolve, reject) => {
        ...
        video.src = apiEndpoint + "media/" + story.media + ".mp4"
      ...
      response["media"] = new Promise((resolve, reject) => {
        ...
        video.playsInline = true;
        video.controls = false;
        video.poster = apiEndpoint + "media/" + story.media + ".jpg" 
        resolve(video);
      })
    }
    return response
